    ...
    try {
        ...
        if (item[0].name === tableName) {
            ...
            if (record?.status === downloadStatus.DOWNLOADED) {
                ...
                const modifiedData = tableRecord.filtered('syncParameters.isDeleted = true OR syncParameters.requireSync = true OR syncParameters.errorInSync = true')
                ...
                let postData = configParam(item[0],staffPositionId,record.lastSync,Array.from(modifiedData));
                //console.log("postData - ",postData);
                const response = await NetworkService.post(item[0].syncApiPath,postData);
                console.log("response--- ",JSON.stringify(response,null,2));
                if (response?.status === Constants.HTTP_OK) {
                    await commonSyncRecordCRUDMethod(item[0],response.data)
                    .then((res) => {
                        resultArray = [...resultArray, ...res];
                        console.log("commonSyncRecordCRUDMethod ",resultArray);
                    });
                    const updatedRecord = await Operations.getAllRecord(item[0].schema[0]);
