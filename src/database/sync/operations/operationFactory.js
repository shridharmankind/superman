import * as CommonSyncMethod from '../commonSyncMethods';
        ...
        await getDBInstance().write(() => {
            data.forEach(object => {
                ...
                if (existingRecord == undefined || existingRecord == null) {
                  ...
                  recordNotExist(item, schema, existingRecord, object, operationObject).then(result => {
                    resultArray.push(result);
                  });
                } else {
                  //case 2
                  //console.log('record exist');
                  recordExist(item, schema, existingRecord, object, operationObject).then(result => {
                    resultArray.push(result);
                  });
                }
...
const recordNotExist = async (item, schema, existingRecord, object, operationObject) => {
    try {
      if (existingRecord == undefined || existingRecord == null) {
        ...
        if (
          ...
          //case 1.2
          //entirely new record
          return operationObject.createNewRecord(dbInstance, schema, object)
        } else {
          //case 1.3
...
const recordExist = async (item, schema, existingRecord, object, operationObject) => {
    try {
      let result = '';
      /**
