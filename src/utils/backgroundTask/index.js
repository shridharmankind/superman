...
export const setOnDemandSyncStatusRunning = async () => {
  const getAsyncValue = await AsyncStorage.setItem(Constants.BACKGROUND_TASK.ON_DEMAND_TASK_NAME, Constants.BACKGROUND_TASK.RUNNING);
  return getAsyncValue;
}

export const setOnDemandSyncStatusNotRunning = async () => {
  const getAsyncValue = await AsyncStorage.setItem(Constants.BACKGROUND_TASK.ON_DEMAND_TASK_NAME, Constants.BACKGROUND_TASK.NOT_RUNNING);
  return getAsyncValue;
}

export const getOnDemandSyncStatus = async () => {
  const getAsyncValue = await AsyncStorage.getItem(Constants.BACKGROUND_TASK.ON_DEMAND_TASK_NAME);
  return getAsyncValue;
}

export const getBackgrounTaskValue = async () => {
  const getAsyncValue = await AsyncStorage.getItem(Constants.BACKGROUND_TASK.TASK_NAME);
  return getAsyncValue;
}
...
export const setRunningBackgrounTaskValue = async () => {
  ...
  if (getAsyncValue === Constants.BACKGROUND_TASK.NOT_RUNNING) {
    await AsyncStorage.setItem(Constants.BACKGROUND_TASK.TASK_NAME, Constants.BACKGROUND_TASK.RUNNING);
...
const setBackgroundTask = async () => {
  try {
    await NetInfo.fetch().then( async (state) => {
      let getCurrentBackgrounStatus = await AsyncStorage.getItem(Constants.BACKGROUND_TASK.TASK_NAME);
      if (state.isConnected && getCurrentBackgrounStatus == Constants.BACKGROUND_TASK.NOT_RUNNING) {
        const accessToken = await KeyChain.getAccessToken();
        let getCurrentAsyncStorage = await AsyncStorage.getItem(Constants.BACKGROUND_TASK.TASK_NAME);
        if (accessToken && getCurrentAsyncStorage === Constants.BACKGROUND_TASK.NOT_RUNNING) {
          console.log(` [${Constants.BACKGROUND_TASK.TASK_NAME}] STARTED ${Constants.BACKGROUND_TASK.RUNNING}`);
          getCurrentAsyncStorage = await setRunningBackgrounTaskValue();
          console.log(`[${Constants.BACKGROUND_TASK.TASK_NAME}] STATUS:  `, getCurrentAsyncStorage);
  ...
  try {
    ...
    if(isDemandSync == Constants.BACKGROUND_TASK.RUNNING){
      if(resultArray.length == 0){
        ...
        showToastie(Constants.TOAST_TYPES.SUCCESS, Strings.backgroundTask.toastBtns.successMessage);
      }
      if (resultArray && resultArray.includes(CONFLICT)) {
        showToastieWithButton(Constants.TOAST_TYPES.WARNING, Strings.backgroundTask.toastBtns.conflictMessage);
      } else if (resultArray && resultArray.includes(FAILURE)) {
        showToastieWithButton(Constants.TOAST_TYPES.ALERT, Strings.backgroundTask.toastBtns.failureMessage);
